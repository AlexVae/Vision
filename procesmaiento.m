...
% Last Modified by GUIDE v2.5 25-May-2019 12:54:27
...
% handles    structure with handles and user data (see GUIDATA)
  global vid
  set(handles. ImagenFiltro,'Visible', 'On');
  imf=GS(getsnapshot(vid));
  imf=grises(imf);
  imf=Histograma(imf);
  [MascaraFiltro , D, E]= Ideal(imf);
  imf =(abs(ifft2(D)));
  axes(handles.ImagenFiltro)
  imshow(imf);
  
  set(handles. imagen,'Visible', 'On');
  img=GS(getsnapshot(vid));
%img=imread('Omar2.jpg');
img=grises(img);
img=Histograma(img);
%img=imread(img);
